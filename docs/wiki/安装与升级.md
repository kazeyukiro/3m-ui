...
每个 Release 都经过两层校验：`SHA256SUMS`（完整性，默认强制）+ cosign keyless 签名（真实性，OPT-IN）。详见 [[验证Release签名]]。
...
默认监听：**`0.0.0.0:8080`**（可用 `PANEL_PORT`、面板「系统设置 → 面板 / NAT」或 `config.yaml` 修改，见 [[NAT与面板端口]]）。
