...
10. **供应链验证**：从 Release 下载资产后，用 `SHA256SUMS` 校验完整性（安装器默认 fail-closed）；推荐再启用 cosign keyless 签名验证（`THREE_M_UI_VERIFY_COSIGN=1`）证明资产由本项目 `release.yml` workflow 签发。详见 [[验证Release签名]]。**切勿**用 `THREE_M_UI_INSECURE=1` 跳过校验，除非你明确理解并接受风险。
